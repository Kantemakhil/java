...
public class Bookmark {
	 ...
	 *     <w:rPr>
	 *       <w:rFonts w:hAnsi="Engravers MT" w:ascii="Engravers MT" w:cs=
	"Arimo"/>
	...
	private void insertAfterBookmark(XWPFRun run) {
		...
		// have
		// matching id numbers.
		bookmarkStartID = _ctBookmark.getId().intValue();
		...
		nextNode = _ctBookmark.getDomNode();
		while (bookmarkStartID != bookmarkEndID) {
			...
			// is encountered converting the id to an int value, the while loop
		...
		if (insertBeforeNode != null) {
			_para.getCTP().getDomNode().insertBefore(run.getCTR().getDomNode(), insertBeforeNode);
		}
	}
	...
	private void insertBeforeBookmark(XWPFRun run) {
		...
		insertBeforeNode = _ctBookmark.getDomNode();
	...
	private void replaceBookmark(XWPFRun run) {
		...
		_para.getCTP().getDomNode().insertBefore(run.getCTR().getDomNode(), nextNode);
	}
	...
	private void deleteChildNodes(Stack<Node> nodeStack) {
		...
		for (int i = 1; i < nodeStack.size(); i++) {
			...
			if (toDelete.getNodeName().contains(Bookmark.BOOKMARK_START_TAG)) {
				...
				if (!inNestedBookmark) {
					_para.getCTP().getDomNode().removeChild(toDelete);
				}
			}
		}
	}
	...
	private StringBuilder getTextFromBookmark() throws XmlException {
		...
		// bookmark (to make looping easier) and initialize the StringBuilder.
		startBookmarkID = _ctBookmark.getId().intValue();
		nextNode = _ctBookmark.getDomNode();
	...
	private String getTextFromChildNodes(Node node) throws XmlException {
		...
		// run to contain zero, one or more text nodes - and recover the text
		...
		for (int i = 0; i < numChildNodes; i++) {
			...
			if (childNode.getNodeName().equals(Bookmark.TEXT_NODE_NAME)) {
				...
				// If the node reports it's type as text, then simply call the
	...
	private void handleBookmarkedCells(String bookmarkValue, int where) {
		...
		List<XWPFParagraph> paraList = _tableCell.getParagraphs();
		for (int i = 0; i < paraList.size(); i++) {
			_tableCell.removeParagraph(i);
		}
		XWPFParagraph para = _tableCell.addParagraph();
