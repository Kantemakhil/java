...
public class Row2BeanRowMapper<T> implements RowMapper<T> {
	...
	private static Logger logger = LogManager.getLogger(Row2BeanRowMapper.class.getName());
	...
	private static class MappingInfo {
		...
		public boolean equals(final Object o) {
			...
			return Objects.equals(sql, that.sql) && Objects.equals(type, that.type);
		}
	...
	public static <M> RowMapper<M> makeMapping(final String sql, final Class<M> type,
			final Map<String, FieldMapper> mappings) {
	...
	private FieldMapper getAdditionalPropertiesFieldMapper(final T bean, final Object value, final String fieldName) {
		return new FieldMapper(FieldMapper.ADDITIONAL_PROPERTIES, null, field -> {
			try {
				...
				logger.warn("Failure adding the field " + fieldName + " on \"" + FieldMapper.ADDITIONAL_PROPERTIES
						+ "\" " + type.getName());
			}
			return null;
		});
