...
left join (
	select
      ...
      where
		...
		STRING_AGG((select profile_value from system_profiles where profile_code = ''PHONE_PREFIX'')::text || '''' || to_char(replace(PHONE_NO, '' ''::text, ''''::text)::bigint,(select profile_value from system_profiles where profile_code = ''PHONE'')::text), '','' order by OWNER_ID) as SCH_TAPDSPHN,
