...
@Repository
public class TagOffApV1RepositoryImpl extends RepositoryBase implements TagOffApV1Repository {
	Logger logger = LogManager.getLogger(TagOffApV1RepositoryImpl.class);

	@Override
	public Integer prIns(final OffenderActionPlans targetObj) {
		final String sql = getQuery("TAG_OFF_AP_V1_OFFENDER_ACTION_PLANS_PRINS");
	...
	public Integer prDel(final OffenderActionPlans lrOldRec) {
		final String sql = getQuery("TAG_OFF_AP_V1_OFFENDER_ACTION_PLANS_PRDEL");
		int[] returnArray = new int[] {};
		final List<SqlParameterSource> parameters = new ArrayList<SqlParameterSource>();
	...
	public Integer prUpd(final OffenderActionPlans targetObj) {
		final String sql = getQuery("TAG_OFF_AP_V1_OFFENDER_ACTION_PLANS_PRUPD");
		int[] returnArray = new int[] {};
		final List<SqlParameterSource> parameters = new ArrayList<SqlParameterSource>();
		parameters.add(new BeanPropertySqlParameterSource(targetObj));
		try {
